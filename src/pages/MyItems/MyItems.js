...
const MyItems = () => {
  ...
  useEffect(() => {
    const userItems = items?.filter(
      (item) => item.supplier === (user?.email).split('@')[0]
    );
  ...
  const handleDeleteButton = (_id) => {
    ...
    if (proceed) {
      const url = `https://fruits-valley-wirehouse-server-djpdmk4e2.vercel.app/services/${_id}`;
      fetch(url, {
        method: 'delete'
      })
        .then((res) => res.json())
        .then((data) => {
          if (data.deletedCount > 0) {
            ...
            const userRemainingItems = userStoredItems?.filter(
              (userStoredItem) => userStoredItem._id !== _id
            );
